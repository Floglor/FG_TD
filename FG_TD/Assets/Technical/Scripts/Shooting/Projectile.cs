...
namespace Shooting
{
    public abstract class Projectile : MonoBehaviour
    {
        ...
        public bool chainChangesDamage;
        public int damageChangeFlat;
        public int damageChangePercentage;
        ...
        protected void DamageWithoutDestroy(GameObject enemy, bool magical)
        {
            ...
            if (effects != null)
                foreach (Effect effect in effects.Where(effect => effect.appliesOnEnemy))
                {
                    ApplyEffectsOnEnemy(enemyObj, effect);
                }

            if (motherTower == null) {
                Debug.Log("VAR");}
            if (motherTower.statEffectObj != null)
                if (enemyObj.type == Type.Boss)
                    motherTower.statEffectObj.AddDamage(damage + penetration + disruption);
        }
        ...
        private void ApplyEffectsOnEnemy(Enemy enemy, Effect effect)
        {
            
            if (effect.GetType() == typeof(ExplodeOnDeath))
            {
                ExplodeOnDeathObj onDeath = new ExplodeOnDeathObj(effect as ExplodeOnDeath);
                
                int[] damageTypes = {damage, penetration, disruption};
                onDeath.flatDamage += (int) ((float) damageTypes.Max()/100 * onDeath.towerDamagePercent);
                enemy.appliedEffects.Add(onDeath);
                return;
            }
        ...
        protected void SpawnLinearWaves(Transform transformPoint)
        {
            ...
            if (rail != null)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    linearWaveScript.motherTower = motherTower;
