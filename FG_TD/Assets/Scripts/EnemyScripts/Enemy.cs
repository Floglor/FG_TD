...
public class Enemy : MonoBehaviour
{
   ...
   // public TextMeshProUGUI shieldText;
    ...
    public List<EffectObj> appliedEffects;
    ...
    private void Start()
    {
        ...
        appliedEffects = new List<EffectObj>();
    ...
    private void TimedBuffsUpdate()
    {
        ...
        {
            if (!flyBuffInstances[i].timed) continue;
        ...
        {
            if (!regenBuffInstances[i].timed) continue;
        ...
        {
            if (!moveSpeedBuffInstances[i].timed) continue;
        ...
        {
            if (appliedEffects[i].isTimedDebuff)
            {
                if (appliedEffects[i].GetType() == typeof(ExplodeOnDeath))
                {
                    ExplodeOnDeathObj onDeath = appliedEffects[i] as ExplodeOnDeathObj;
                    
                    onDeath.trueCounter -= Time.deltaTime * PlayerStats.instance.gameSpeedMultiplier;
                    
                    if (!(onDeath.trueCounter <= 0)) continue;

                    appliedEffects.Remove(onDeath);
                }
            }
        }
    ...
    public void Die()
    {
        ...
        ExplodeOnDeathObj explodeOnDeath = CheckForExplodesOnDeath();

        if (explodeOnDeath != null)
        {
            Explode(explodeOnDeath);
        }
       
     
        SoundMaster.instance.Play(RandomNumber.randomNumber.random.Next(1,6), pitch:  (float) (RandomNumber.randomNumber.random.NextDouble() * (1.7 - 0.7) + 0.7));
    }

    private ExplodeOnDeathObj CheckForExplodesOnDeath()
    {
        if (appliedEffects.IsNullOrEmpty()) return null;

        int biggestDamage = 0;
        ExplodeOnDeathObj biggestExplode = null;

        
        
        

        foreach (ExplodeOnDeathObj onDeath in from appliedEffect in appliedEffects where appliedEffect.GetType() == typeof(ExplodeOnDeathObj) 
            select appliedEffect as ExplodeOnDeathObj into onDeath where onDeath.flatDamage + (float) startHealth / 100 * onDeath.enemyHPPercent > biggestDamage select onDeath)
        {
            biggestDamage = onDeath.flatDamage + startHealth / 100 * onDeath.enemyHPPercent;
            biggestExplode = onDeath;
        }

        
        return biggestExplode;
    }

    private void Explode(ExplodeOnDeathObj explodeOnDeath)
    {
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, explodeOnDeath.explodeRadius);
        foreach (Collider2D collider in colliders)
        {
            if (collider.CompareTag("Enemy"))
            {
                Damage(collider.transform.gameObject, explodeOnDeath.flatDamage + (startHealth / 100) * explodeOnDeath.enemyHPPercent);
            }
        }
    }

    private void Damage(GameObject enemy, int flatDamage)
    {
        ...
        enemyObj.TakeDamage(flatDamage, 0, 0);
