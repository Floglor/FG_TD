...
namespace Shooting
{
    ...
    public class TowerAI : MonoBehaviour
    {
        ...
        [BoxGroup("Locked Stats")] public int lockedDamage;
        [BoxGroup("Locked Stats")] public float lockedFireRate;
        [BoxGroup("Locked Stats")] public float lockedRange;

        [BoxGroup("Free Stats")] public int freeDamage;
        [BoxGroup("Free Stats")] public float freeFireRate;
        [BoxGroup("Free Stats")] public float freeRange;

        private SoundMaster soundMaster;
        [Header("Upgrades")] public List<UpgradeVariant> upgradeVariants;

        [Header("DAMAGE MODIFIER")] [Range(-100, 200)]
        public int damageModifier;

        [Header("ATTACKSPEED MODIFIER")] [Range(-100, 200)]
        ...
        [ConditionalField(nameof(isChainLighting))]
        public int startDamageChangeFlat;

        public int damageChangeFlat { get; set; }

        [ConditionalField(nameof(isChainLighting))]
        public int startDamageChangePercentage;

        public int damageChangePercentage { get; set; }
        ...
        [Header("EssenceGain")] public bool isFreeEssenceGain;
        ...
        [ConditionalField(nameof(isEnemySuckGain))]
        public int maxSuckAmount;

        [ConditionalField(nameof(isEnemySuckGain))]
        public bool isFullOfEssence;

        [ConditionalField(nameof(isEnemySuckGain))]
        public float startSuckAOE;

        [ConditionalField(nameof(isEnemySuckGain))]
        public float suckAOE;
        ...
        [Header("Target Priority")] public bool shieldsFirstOverride;
        public bool oneShotTargetFistOverride;
        ...
        [DisableInPrefabs] [FormerlySerializedAs("totalCost")]
        public int totalCostRun;

        public int totalCostOnStart;

        [Header("Buff Relation Logic (NOT USED)")]
        public bool notAffectedByAttackAuras;

        public bool notAffectedByAttackSpeedAuras;
        public bool notAffectedByRadiusAuras;

        [Header("Description")] public TowerTypeSmallStatList typeSmallStatList;
        ...
        public StealStatEffectObj statEffectObj { get; set; }
        ...
        private void Start()
        {
            soundMaster = SoundMaster.instance;
            ...
            if (falseCrits == null)
            {
                falseCrits = new List<FalseCrit>();
            }

            if (statEffectObj != null)
            {
                statEffectObj.tower = this;
            }

            foreach (FalseCrit falseCrit in falseCrits
                .Where(falseCrit => falseCrit.effect.GetType() == typeof(StealStatEffect))
                .Where(falseCrit => statEffectObj == null))
            {
                statEffectObj = new StealStatEffectObj((StealStatEffect) falseCrit.effect, this);
            }
            ...
            damageChangeFlat = startDamageChangeFlat;
            damageChangePercentage = startDamageChangePercentage;
            ...
            IntegerStatRecalculate(Stats.IntStatName.Damage);
            FloatStatRecalculate(Stats.FloatStatName.AttackSpeed);
        }
            ...
            int biggestDebuffPercentageFromAura, float totalStatFlatDebuff, int totalStatPercentageDebuff)
        {
            switch (floatStatName)
            {
                case Stats.FloatStatName.AttackSpeed:
                    ...
                    fireRate = fireRate / 100.0f * (100.0f + attackSpeedModifier);
                    break;
                ...
                case Stats.FloatStatName.EnemySuckAOE:
                    suckAOE = startSuckAOE;
                    suckAOE = FloatStatFormula(suckAOE, totalStatFlatBuff,
                        totalStatPercentageBuff,
                        biggestBuffFlatFromAura, biggestBuffPercentageFromAura, biggestDebuffFlatFromAura,
                        biggestDebuffPercentageFromAura, totalStatFlatDebuff, totalStatPercentageDebuff,
                        FindExternalBuff(Stats.FloatStatName.EnemySuckAOE));
                    break;
                default:
        ...
        public int FindExternalBuff(Stats.IntStatName intStatName)
        {
            ...
            Stats.FloatStatName floatStatName)
        {
            ...
            if (statEffectObj != null)
            {
                if (!statEffectObj.farmableStatsFloat.IsNullOrEmpty())
                {
                    totalStatFlatBuff += statEffectObj.farmableStatsFloat
                        .Where(farmableStatFloat => farmableStatFloat.floatStatName == floatStatName)
                        .Sum(farmableStatFloat => farmableStatFloat.statCounter);
                }
            }
            ...
            int totalStatPercentageDebuff)
        {
            ...
            switch (intStatName)
            {
                case Stats.IntStatName.Damage:
                    ...
                    damage = (int) Math.Round(damage / 100.0f * (100.0f + damageModifier));
                    break;
                case Stats.IntStatName.PenetrateAmount:
                    ...
                    penetration = (int) Math.Round(penetration / 100.0f * (100.0f + damageModifier));
                    break;
                ...
                case Stats.IntStatName.Disruption:
                    ...
                    disruption = (int) Math.Round(disruption / 100.0f * (100.0f + damageModifier));
                    break;
            ...
            Stats.IntStatName intStatName)
        {
            ...
            if (statEffectObj != null)
            {
                if (!statEffectObj.farmableStatsInt.IsNullOrEmpty())
                {
                    statFlatBuffValue += statEffectObj.farmableStatsInt
                        .Where(farmableStatInt => farmableStatInt.intStatName == intStatName)
                        .Sum(farmableStatInt => farmableStatInt.statCounter);
                }
            }
        ...
        private void ApplyRadiusBuff(TowerAI towerAi)
        {
            if (radiusBuffPercentage != 0)
                towerAi.floatTowerBuffs.Add(new FloatTowerBuff(id, false, true, true, Stats.FloatStatName.Range,
                    radiusBuffPercentage));

            if (radiusBuffFlat != 0)
                towerAi.floatTowerBuffs.Add(new FloatTowerBuff(id, false, false, true, Stats.FloatStatName.Range,
                    radiusBuffFlat));
        ...
        {
            if (percentageAttackBuff != 0)
                towerAi.integerTowerBuffs.Add(new IntegerTowerBuff(id, false, true, true,
                    Stats.IntStatName.PenetrateAmount,
                    percentageAttackBuff));

            if (flatAttackBuff != 0)
                towerAi.integerTowerBuffs.Add(new IntegerTowerBuff(id, false, false, true,
                    Stats.IntStatName.PenetrateAmount,
                    flatAttackBuff));
        ...
        private void SpawnGroundPoints()
        {
            ...
            foreach (GroundProjectile groundProjectile in groundProjectiles)
            {
                ...
                groundProjectile.motherTower = this;
        ...
        {
            // Debug.Log("boom");
            if (oneShotTargetFistOverride)
            {
                SingleTargetOneShotEnemyUpdateTarget();
            }

            else if (shieldsFirstOverride)
            {
                ShieldTargetSearch();
            }
            else
        ...
        private void ShieldTargetSearch()
        {
            List<GameObject> enemies = FindEnemies();
            ...
            List<GameObject> shieldedEnemies =
                (from enemy in enemies let enemyAI = enemy.GetComponent<Enemy>() where enemyAI.shield > 0 select enemy)
                .ToList();

            if (shieldedEnemies.IsNullOrEmpty())
            {
                NotOneshotTargetSearch();
            }
            else
            {
                NotOneshotTargetSearch(shieldedEnemies);
            }
        }

        private void NotOneshotTargetSearch(List<GameObject> enemies = null)
        {
            if (enemies == null)
                enemies = FindEnemies();
        ...
        private void AssignChain(Projectile bullet)
        {
            ...
            bullet.damageChangeFlat = damageChangeFlat;
            bullet.damageChangePercentage = damageChangePercentage;
        }
